class User < ActiveRecord::Base
  ...
  has_many :campaings, dependent: :nullify
  ...
  #we are accessing outside
  def self.find_or_create_with_twitter(omniauth_data)
    #find user where the provider is twitter and the user id is from omniauth data
    user = User.where(provider: "twitter", uid: omniauth_data["uid"]).first
    #if user is not found we need to create the user
    unless user
